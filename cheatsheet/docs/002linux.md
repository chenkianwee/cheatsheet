...
1. Install the zotero plugin manually. Go to where your zotero is installed. If installed using snap it will be here '/snap/zotero-snap/current/integration/libreoffice/Zotero_LibreOffice_Integration.oxt'

2. Copy and paste the file to Desktop

3. Go to libreoffice Writer -> Tools -> Extension -> Add select the file Zotero_LibreOffice_Integration.oxt and install it
...
2. <del>install it with the following command</del>
    ```
    sudo apt-get install libqt5webkit5
    sudo dpkg -i GlobalProtect_UI_deb-6.2.1.1-276.deb
    ```
3. <del>Once installed go to the installed directory</del>
    ```
