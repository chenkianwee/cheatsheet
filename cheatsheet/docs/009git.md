...
- go to a git repository folder in your computer. submodule_url is the url of the git repo, and path is the file path you want to clone the repo into. 
```
git submodule add <submodule_url> <path>
```

- to git commit your submodule you will need to go into the submodule folder to do the commit

### cloning a repo with submodules
- clone the repo, the submodules are not automatically clone
```
git clone <repo url>
```

- next you will need to do a submodule init
```
git submodule init
```

- the finally run this 
```
git submodule update
```

### Checking out submodules in workflow files

- make sure to specify to fetch all submodules
  ```
  jobs:
    build:
