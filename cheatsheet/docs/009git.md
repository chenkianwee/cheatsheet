...
2. copy all the files from the _build folder into the root of the branch
```
cp -r _build/html/* .
``` 
3. add all the files to the commit. Make sure to add the .gitignore file to ignore files you are not committing.
```
git add .
...
5. configure the gh-pages setting to deploy from the gh-pages branch 
